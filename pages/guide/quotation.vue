  ...
  <section v-if="!$fetchState.pending && !$fetchState.error" id="top">
    ...
    <div class="sec__inner d-lg-flex py-16">
      ...
      <div class="content guide__content ml-lg-10">
        <guideline-section-title section-type="quotation"></guideline-section-title>
        <guideline-section
...
export default {
  data() {
    return {
      breadCrumbs: [],
      guideList: items.quotation,
      guideSections: items.quotation.sections,
    }
  },
  fetch() {
  ...
  head () {
    return {
      title: "レンタルご利用ガイド",
      meta: [
        { hid: 'description', name: 'description', content: 'レンタルご利用ガイド - 見積もりについて - | 大阪 東京 名古屋 京都での映像機器・音響機器のレンタルや学会・展示会・式典・試写会などのイベント制作・運営/HDV映像・音楽制作/VJ機器・HDVカメラなどクリエイターに向けの映像設備のプランニング・施工/デジタルサイネージやITネットワーク構築など映像・音響・レンタル・販売・設備に関することは80年の歴史を持つ(株)タケナカにご相談下さい。' }
      ]
    }
  },
  updated() {
  ...
  methods: {
    setBreadCrumbs() {
      ...
      this.$store.commit('breadCrumbs/addList', { name: 'レンタルご利用ガイド - 見積もりについて -', path: '/guide/quotation' })
